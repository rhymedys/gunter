    ...
    <div @scroll="ganteview_scroll" class="ganteview">
      <div :style="{width:header_width+'px'}" class="ganteview-header" >
        <div class="ganteview-toptime">
          <div class="ganteview-headercell" :title="item.date" v-for="item in top_time_data" :key="item.date" :style="{width:item.width+'px',left:item.left+'px'}">{{item.date}}</div>
        </div>
        <div class="ganteview-bottomtime">
          <div class="ganteview-headercell" :title="item.title" v-for="item in bottom_time_data" :key="item.title" :style="{width:item.width+'px',left:item.left+'px'}">{{item.date}}</div>
        </div>
      </div>
      <gante-gc-item :th_data="th_data" :style="{width:header_width+'px'}" v-if="show_item" class="ganteview-content-box" :gante_data="gante_data" >
        <div v-for="item in bottom_time_data" class="ganteview-column" :style="{width:item.width+'px',left:item.left+'px',background:item.is_rest?'#f5f7fa':null}" :key="item.title"></div>
      </gante-gc-item>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ganteview_scroll(e) {
      ...
      document.getElementsByClassName("ganteview-header")[0].style.left =
    ...
    init(time_mode) {
      ...
      switch (time_mode) {
        ...
        case 2:
          ...
          this.first_day = new Date(
            ...
            start_time.getMonth(),
            start_time.getDate() - ((start_time.getDay() + 7) % 7)
          );
          break;
        case 3:
      ...
      if (this.end_time) {
        ...
        if (
          gante_width * time >=
          all_time - new Date(this.first_day).getTime()
        ) {
    ...
    get_top_time(mode, first_time, time, all_width, time_mode) {
      ...
      for (let i = 0; i <= all_width; ) {
        ...
        if (mode == 1) {
          //        月的形式
          first_time = new Date(
            first_time.getFullYear(),
            first_time.getMonth() + 1,
            1
          );
        } else if (mode == 2) {
          //         年的形式
      ...
      for (let i = 0; i <= all_width; ) {
        ...
        if (mode == 1) {
          ...
          first_time = new Date(
            first_time.getFullYear(),
            first_time.getMonth(),
            first_time.getDate() + 7
          );
          title =
            this.format(old_time, 1) +
            "~" +
            this.format(
              new Date(
                first_time.getFullYear(),
          ...
          title =
            ...
            "~" +
            this.format(
              new Date(first_time.getFullYear(), first_time.getMonth(), 0),
              1
            );
        } else if (mode == 5) {
          first_time = new Date(
            first_time.getFullYear(),
            first_time.getMonth() + 6,
            1
          );
