      ...
      if (this.th.edit || this.th.chooseTime) {
        ...
        if (this.th.chooseTime) {
          var time = "",
            ...
            gante_table_box = document.getElementsByClassName(
              "gante-table-box"
            )[0],
            ganteview = document.getElementsByClassName("ganteview")[0];
          if (
            value.params[this.key_value] == "" ||
            value.params[this.key_value] == null
          ) {
            time = new Date();
          } else {
          ...
          this.$emit("change-calendar", {
            ...
            change_value: {
              ...
              argument: [
                ...
                {
                  argument: this.th.time_mode == 1 ? "start_time" : "end_time",
                  mode: true
                }
              ]
            },
      ...
      if (this.$refs.gante_input.value != this.td_data.params[this.key_value]) {
        this.$emit("change", {
          change_value: {
            id: id,
            value: [
              { argument: this.key_value, value: this.$refs.gante_input.value }
            ]
          }
        });
      }
